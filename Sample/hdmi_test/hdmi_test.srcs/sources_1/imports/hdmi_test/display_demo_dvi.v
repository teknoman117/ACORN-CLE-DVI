...
// (C)2021 Nathaniel Lewis, Edits for Acorn CLE-215+
...
module display_demo_dvi(
    input wire clk_p,               // board clock: 200 MHz on Acorn (differential)
    input wire clk_n,
    ...
    // Nathan: differential clock input for Acorn CLE-215+
    IBUFDS IBUFDS_inst (
        .O(CLK),
        .I(clk_p),
        .IB(clk_n)
    ...
    // Nathan: double DIV_MASTER constants for 200 MHz Acorn clock,
    //         the original timings assumed a 100 MHz clock
    display_clocks #(               // 640x480  800x600 1280x720 1920x1080
        .MULT_MASTER(37.125),       //    31.5     10.0   37.125    37.125
        .DIV_MASTER(10),            //      10        2       10        10
        .DIV_5X(1.0),               //     5.0      5.0      2.0       1.0
        .DIV_1X(5),                 //      25       25       10         5
        .IN_PERIOD(5.0)             // 200 MHz = 5 ns
    )
    display_clocks_inst
    (
       .i_clk(CLK),
    ...
    /*test_card_simple #(
        .H_RES(1920)    // horizontal resolution
    ) test_card_inst (
        .i_x(sx),
        .o_red(red),
        .o_green(green),
    ...
    /*test_card_squares #(
        .H_RES(1920),   // horizontal resolution
        .V_RES(1080)    // vertical resolution
    )
    test_card_inst (
        .i_x(sx),
        .i_y(sy),
    ...
    /*localparam GRAD_STEP = 3;  // step right shift: 480=2, 720=2, 1080=3
    ...
    // Nathan: create a test card with some animation
    // Test Card: Animation - ENABLE ONE TEST CARD INSTANCE ONLY
    test_card_animation  #(
        .H_RES(1920),   // horizontal resolution
        .V_RES(1080)    // vertical resolution
    )
    ...
    // Nathan: count up on the user LEDs for some sign of life
